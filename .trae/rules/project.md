...
| 域名元信息同步 | 同步域名注册时间（`registrationDate`）与 NameServer 列表（`nameServers`），支持新增与更新已存在域名 |
...
| 系统配置 | 集中管理 SMTP 邮件服务和 NS 检查 DNS 服务器配置 |
| NS 检查服务器 | 支持 UDP / TCP / DoT / DoH 四种协议的域名 NS 记录查询配置，含手动检测功能 |
